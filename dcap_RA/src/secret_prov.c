...
int secret_provision_start(const char* in_servers, const char* in_ca_chain_path,
    ...
    if (!ca_chain_path) {
        ret = -ENOMEM;
        printf("[error] ca_chain_path returned %d\n", ret);
        goto out;
    }
    ...
    if (!servers) {
        ret = -ENOMEM;
        printf("[error] servers returned %d\n", ret);
        goto out;
    }
    printf("servers = %s \n", servers);
    ...
    for (str1 = servers; /*no condition*/; str1 = NULL) {
        ...
        printf("connected_port = %s \n", connected_port);
        if (!connected_port)
            continue;

        printf("mbedtls_net_connect connected_addr= %s,connected_port=%s \n",connected_addr,connected_port );
        ret = mbedtls_net_connect(&g_verifier_fd, connected_addr, connected_port,
                                  MBEDTLS_NET_PROTO_TCP);
        if (!ret){
            printf("mbedtls_net_connect SUCCESS ret= %d \n", ret);
            break;
        }
    }
    printf("servers = %s \n", servers);
    ...
    if (ret != 0) {
        ...
        printf("[error] mbedtls_x509_crt_parse_file ca_chain_path=%s \n",ca_chain_path);
        goto out;
    }
    ...
    if (ret < 0) {
        printf("[error] mbedtls_x509_dn_gets returned %d\n", ret);
        goto out;
    }
    printf("crt_issuer = %s \n", crt_issuer);
    ...
    ret = ra_tls_create_key_and_crt(&g_my_ratls_key, &g_my_ratls_cert, quote,quote_size);
    if (ret < 0) {
        printf("[error] ra_tls_create_key_and_crt returned %d\n", ret);
        goto out;
    }
    printf("[OK] ra_tls_create_key_and_crt returned %d\n", ret);
    ...
    while (ret < 0) {
        ...
        if (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE) {
            printf("[error] MBEDTLS_ERR_SSL_WANT_READ MBEDTLS_ERR_SSL_WANT_WRITE \n");
            continue;
        }
        if (ret < 0) {
    ...
    if (flags != 0) {
        ...
        printf("[error] mbedtls_ssl_get_verify_result returned %d\n", ret);
        goto out;
    }
    ...
    if (ret < 0) {
        printf("[error] secret_provision_write returned %d\n", ret);
        goto out;
    }
    ...
    if (ret < 0) {
        printf("[error] secret_provision_read returned %d\n", ret);
        goto out;
    }

    if (memcmp(buf, SECRET_PROVISION_RESPONSE, sizeof(SECRET_PROVISION_RESPONSE))) {
        ret = -EINVAL;
        printf("[error] SECRET_PROVISION_RESPONSE returned %d\n", ret);
        goto out;
    }
    ...
    if (received_secret_size > INT_MAX) {
        ret = -EINVAL;
        printf("[error] received_secret_size returned %d\n", ret);
        goto out;
    }
    ...
    if (!provisioned_secret) {
        ret = -ENOMEM;
        printf("[error] provisioned_secret returned %d\n", ret);
        goto out;
    }
