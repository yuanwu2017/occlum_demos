...
//#define CERT_SUBJECT_NAME_VALUES "C=NL,O=PolarSSL,CN=localhost"
...
static int generate_x509(mbedtls_pk_context* pk, const uint8_t* quote, size_t quote_size,
    ...
    printf("cert_timestamp_not_before=%s \n",cert_timestamp_not_before);
    ...
    printf("cert_timestamp_not_after=%s \n",cert_timestamp_not_after);
...
static int create_x509(mbedtls_pk_context* pk, mbedtls_x509write_cert* writecrt, uint8_t *quote, uint32_t quote_size) {
...
static int create_key_and_crt(mbedtls_pk_context* key, mbedtls_x509_crt* crt, uint8_t** crt_der,
                              size_t* crt_der_size, uint8_t *quote, uint32_t quote_size) {
    int ret;
    ...
    ret = create_x509(key, &writecrt, quote,quote_size);
    if (ret < 0)
        goto out;
...
int ra_tls_create_key_and_crt(mbedtls_pk_context* key, mbedtls_x509_crt* crt, uint8_t *quote, uint32_t quote_size) {
    return create_key_and_crt(key, crt, NULL, NULL,quote,quote_size);
}
...
int ra_tls_create_key_and_crt_der(uint8_t** der_key, size_t* der_key_size, uint8_t** der_crt,
    ...
    ret = create_key_and_crt(&key, NULL, der_crt, der_crt_size,NULL,0);
    if (ret < 0) {
        goto out;
    }
